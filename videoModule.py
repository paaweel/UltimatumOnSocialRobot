...
class VideoModule:
    ...
    def onHumanTracked(self, value):
        ...
        if value != []:
            ...
            imgPath = os.path.join(
                Config().videoPath, self.gameDirName, timestamp + ".jpg"
            )
    ...
    def cropFace(self, npImg, faceInfo):
        ...
        imgSize = max(
            (faceInfo[0][0][3] + 0.10) * self.width,
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        logging.debug(
            "Incorrect number of arguments, required 1 with" "video game directory name"
        )
        sys.exit(5)
